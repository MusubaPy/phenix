...
#ifndef MJPC_TASKS_QUADRUPED_MOD_QUADRUPED_MOD_H_
#define MJPC_TASKS_QUADRUPED_MOD_QUADRUPED_MOD_H_
 ...
 public:
  ...
  explicit ResidualFn(const QuadrupedFlatMod* task)
    ...
    void Residual(const mjModel* model, const mjData* data,
      ...
      struct MotorSelectResult {
        bool valid = false;
        int index = -2;  // -2 = unset, -1 = blended, >=0 discrete index
        double angle = 0.0;
        double motor_proj[3] = {0.0, 0.0, 0.0};
      };

      static MotorSelectResult SelectMotorUsingNormalForTest(
          const double contact_normal[3], const double motors[2][3],
          const double plane_normal[3], double blend_beta = 0.0);

      static bool ComputePlaneFromPointsForTest(const double a[3],
                                                const double b[3],
                                                const double c[3],
                                                double out_normal[3]);

      static bool ComputePerFootResidualForTest(const double force[3],
                                                const double normal[3],
                                                const double hip[3],
                                                const double knee[3],
                                                const double foot[3],
                                                double friction, double out[3]);
  ...
  friend class QuadrupedFlatMod;
    ...
    int biped_type_param_id_  = -1;
      // Alexander cost parameters
      int alex_enabled_param_id_ = -1;
      int alex_power_weight_param_id_ = -1;
      int alex_align_weight_param_id_ = -1;
      int alex_fx_smooth_weight_param_id_ = -1;
    ...
    // Alexander cost state
    mutable double alex_last_fx_[kNumFoot];
    mutable bool alex_initialized_ = false;

    // Compute Alexander cost components per timestep (placed here to keep
    // function isolated inside quadruped_mod). Outputs:
    //  - e_power_abs: sum_t |tau_i * qdot_i|*dt (instantaneous abs power)
    //  - e_align: alignment penalty (radians) between GRF and target joint
    //  - e_fx_smooth: squared change of Fx for smoothing
    //  - grf_out: per-foot GRF vector in world coords (fx,fy,fz)
    void ComputeAlexanderTerms(const mjModel* model, const mjData* data,
...
// No extra aliases needed: this header now declares Mod variants directly.

#endif  // MJPC_TASKS_QUADRUPED_MOD_QUADRUPED_MOD_H_